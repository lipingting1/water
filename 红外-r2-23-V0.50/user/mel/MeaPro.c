...
void MeaDetail(uint8_t chMode)
{
	...
	LCD_Display_String(264,200,"返回",ADDPART1,NORMAL,NORMAL);
	Draw_ArcFrame(0,32,240,184,ADDPART1,NORMAL);
	sprintf(DisBuffer,"CP:总:%d%04d",(uint16_t)(MesCP_Cs.SumNum/10000),(uint16_t)(MesCP_Cs.SumNum%10000));
	...
	LCD_Display_String(80,140,"其他:---",ADDPART1,NORMAL,NORMAL);
...
void MeasureProPage(uint8_t chMode,bit bMeaState) 		//测量成功页面 和 测量中页面		   0x12
{
	...
	if(1==bMeaState){
		...
		if(1 != chMode)LCD_Display_String(264,80,"详情",ADDPART1,NORMAL,NORMAL);
		LCD_Display_String(264,200,"返回",ADDPART1,NORMAL,NORMAL);
		...
		if(1==chMode){
			...
			LCD_Display_String(80,120,"精度:--%",ADDPART1,NORMAL,NORMAL);
			LCD_Display_String(44,144,"I-131:",ADDPART1,NORMAL,NORMAL);
			LCD_Display_String(80,180,"精度:--%",ADDPART1,NORMAL,NORMAL);
		}
	}else{
...
void MeaProDeal(uint8_t chKey,uint8_t chMode)			//测量中处理			0x14
{
	...
	case 3:	Stop_Measure();				//中止
			...
			bMeaResuSta=1;
			MeasureProPage(Get_MeaMode(),(bit)(SystemPara.MeaState&(1<<6)));break; //	判断时间是否到达临界时间 可能需要分情况讨论失败原因
