...
extern uint8_t Yper,Xper,Xpos,Ypos;
extern bit DatUpdFla,bSavPar;
extern Sys_Para SystemPara;
extern Uint32_t	Mes_CPN;
...
void ParaPage(uint8_t chMode)							//参数修改页面  
{
	...
	case 3: LCD_Display_String(48,60,"测 量",ADDPART1,NORMAL,NORMAL);	 
			...
			chParaNum=sprintf(ParaBuffer,"%04d%c%02d%c%04d%c%04d",(uint16_t)(SystemPara.MTime%100),0x0d,(uint16_t)SystemPara.MeasureAcc,0x0d,(uint16_t)SystemPara.Alarm_V,0x0d,(uint16_t)SystemPara.Alarm_Q);
...
void ParPagUpd(uint8_t chKey,uint8_t chMode)			//参数修改操做处理      0x41		KEY		  chSetParMode
{
	...
	case 4: switch(chMode){							  //确认
			...
			case 2: if(0==(SystemPara.MeaState&(1<<4))){						 //固体
						...
						sscanf(DisBuffer,"%04d",(uint16_t *)piPtr);
						Write_EEprom(System_ram,(uint8_t *)&SystemPara.Quality,8,4);
						MeasureProPage(Get_MeaMode(),1);	
					}
					else MeasureBusyPage();
					break;
			case 3:			  //测量参数
			case 4: if(3 == chMode)(uint16_t *)piPtr=&SystemPara.MTime;	  //系统参数
					else{
					...
					for(i=0,j=0;i<chParaNum;i++,j++){
						if((ParaBuffer[i] == 0x0d) | (ParaBuffer[i] == '/') | (ParaBuffer[i] == ':')){
							if((&SystemPara.CalibFactor_I == (float *)piPtr) | (&SystemPara.CalibFactor_Cs == (float *)piPtr)){	 //浮点型
								sscanf(DisBuffer,"%.2f",(float *)piPtr);
								((float *)piPtr)++;
							}else{
								if(		(&SystemPara.Alarm_Q  == (uint16_t *)piPtr) 
									|| 	(&SystemPara.Alarm_V  == (uint16_t *)piPtr)
									||	(&SystemPara.MTime    == (uint16_t *)piPtr)
								...
								{ //整形
									...
									*(uint8_t *)piPtr=(uint8_t)SetValueBuf;
									((uint8_t *)piPtr)++;
									if((&SystemPara.SysTime[2] == (uint8_t *)piPtr)) ((uint8_t *)piPtr)++;
								}
							}
							j=255;
						}else{
					...
					Write_EEprom(System_ram,piPtrShad,k1,k2);
