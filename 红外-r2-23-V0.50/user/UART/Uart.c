...
bit WR_En,bDataRecStar,bCRC,bOveJua=0,UART_flag=0;
...
uint8_t xdata chSendBuffOUTPos,chSendBuffInPos,chSendBuffer[SEND_BUFFER_NUM];
...
uint16_t CRC_Check2(uint8_t *buf , uint8_t count)
{
	uint16_t CRCV = 0xFFFF;
	uint8_t iB ,i ,j;
	for (i = 0; i < count; i++){
...
void PackBag(void) //编辑发送包
{
    ...
    temp = CRC_Check2(&DataBagBuff[8],chTraDatLen+4);   //写入CRC校验码
...
void chRecSuccess(void)
{
	uint8_t *p,i,NowPos;
	bit bSavFla;  
	REN=0;bDataRecStar=0;chRecPtr=0;bOveJua=0;
	p=(uint8_t *)&SystemPara.MeachineAddr+DataBagBuff [11];
	if(0==WR_En){					  		//读寄存器数据
		PackBag();
	}else{
...
void RecDeal(void)
{
	...
	if(SBUF==0X02 || 1==bDataRecStar){
		...
		if(7==chRecPtr){
			if(DataBagBuff [7] != (uint8_t)sum_Check){chAbandonData();SBUF=0x41;}  // | 8!=chRecPtrLRC校验 校验成功继续接受 失败则清除接受标志位
		}
		if(9 == chRecPtr){
...
//				data1=0x61;
		...
		if((chRecLen <= chRecPtr)&(1 == bOveJua)){		 //数据接受完成
			if(0==CRC_Check(&DataBagBuff[8],chDataLen+6))UART_flag=1;//chRecSuccess();
...
////		TI=0;
////		chSendPtr++;
////		if(chSendLen <= chSendPtr)
////			REN=1; //   
////		else{
////			SBUF=DataBagBuff [chSendPtr];
////		}
////	}
//	if (1 == TI){
//		TI = 0;
//		if (chSendBuffOUTPos == chSendBuffInPos){
//			REN=1;
//		}else{
//			SBUF = chSendBuffer[chSendBuffOUTPos++];
//			chSendBuffOUTPos = chSendBuffOUTPos % SEND_BUFFER_NUM;
//		}
//	}
//}
//
void Usart_Debug(const char *fmt,...)//
{
	va_list ap;//typedef char *va_list; va_list是char型的指针
	char xdata chchar[50];//访问外部RAM 两字节对齐
	uint8_t i=0;
	va_start(ap,fmt);//这个函数的功能是，找到第一个可变形参的地址，并把地址赋给ap
	vsprintf(chchar,fmt,ap);//其实这个函数才是核心函数，没研究。。。
//	sendstring(string);//这个函数就是发送字符串函数，通过上一个函数，就把该提取的东西都提取了
	while(chchar[i] != '\0'){
		chSendBuffer[chSendBuffInPos++] = chchar[i++];
		chSendBuffInPos = chSendBuffInPos % SEND_BUFFER_NUM;
		if (chSendBuffInPos == chSendBuffOUTPos){
			chSendBuffInPos--;
		 	break;
		}
	}
