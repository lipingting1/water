	...
	*k=0;jux=0;
	...
	while(2 != *k){	 //NowPos--
		if(0 == *NowPos){	//找到当前读取位置		// 确定当前显示在扇区中的位置并打印到缓冲区内
			Read_EEprom(hwCurRAM,(uint8_t *)&SystemPara.History.Record[*k][0],NowAddr-24,25); //4*7	 /读下个数据指针
...
void UpdHisRec(uint8_t NowPageAddr)	 		//历史记录页面刷新函数			当前显示, Ptr当前页面的显示的行数，FriAddr为当前页面
{
	...
	while(k!=chNum){                                                                   
		...
		LCD_Display_String(96,96+(chNum<<6),ParaBuffer,ADDPART1,NORMAL,NORMAL);//NowShow标识的是当前页面显示的最后一条指令地址  Cs-137:%dBg/Kg	  %c
		...
		if(0 == Cs_Value);
			...
			LCD_Display_String(96,112+(chNum<<6),ParaBuffer,ADDPART1,NORMAL,NORMAL);//NowShow标识的是当前页面显示的最后一条指令地址  Cs-137:%dBg/Kg	  %c
		}
		...
		if(0 == I_Value);
			...
			LCD_Display_String(96,128+(chNum<<6),ParaBuffer,ADDPART1,NORMAL,NORMAL);//NowShow标识的是当前页面显示的最后一条指令地址  Cs-137:%dBg/Kg	  %c
		}
		chNum++;
	}
}
...
bit SaveDeal(uint8_t chMode)
{
	...
	case 3:	Read_EEprom(0,(uint8_t *)&SystemPara.RecordQua,0,2);   //  
			...
			if(0 == SystemPara.RecordQua){
				...
				if(RAMdata == SystemPara.History.Record[0][8]){ //扇区满溢出，从小到大查询出空闲扇区
					...
					while(1){	 //	寻址扇区中的尾结点 返回值NowAddr地址之后的24个值
						NowAddr=NowAddr*25+36;  //只读最后一个数据36=12+24
			...
			Write_EEprom(hwCurRAM ,(uint8_t *)&SystemPara.History.Record[1][0],NowAddr,25); //指针链接。。原来的尾指针0指向新增指针i

//			Read_EEprom(0,(uint8_t *)&sd[8],10,1);	  // 读扇区首存储地址 和 扇区存储区状态表
